...
async def to_code(config):
    ...
    # Fall mode - installation
    if install_angle_x := config.get(CONF_INSTALL_ANGLE_X):
        sens = await sensor.new_sensor(install_angle_x)
        cg.add(parent.set_install_angle_x_sensor(sens))
    if install_angle_y := config.get(CONF_INSTALL_ANGLE_Y):
        sens = await sensor.new_sensor(install_angle_y)
        cg.add(parent.set_install_angle_y_sensor(sens))
    if install_angle_z := config.get(CONF_INSTALL_ANGLE_Z):
        sens = await sensor.new_sensor(install_angle_z)
        cg.add(parent.set_install_angle_z_sensor(sens))
    if install_height := config.get(CONF_INSTALL_HEIGHT):
        sens = await sensor.new_sensor(install_height)
        cg.add(parent.set_install_height_sensor(sens))
    
    # Fall mode - data
    if fall_state := config.get(CONF_FALL_STATE):
        sens = await sensor.new_sensor(fall_state)
        cg.add(parent.set_fall_state_sensor(sens))
    if static_residency := config.get(CONF_STATIC_RESIDENCY):
        sens = await sensor.new_sensor(static_residency)
        cg.add(parent.set_static_residency_sensor(sens))
    if static_residency_time := config.get(CONF_STATIC_RESIDENCY_TIME):
        sens = await sensor.new_sensor(static_residency_time)
        cg.add(parent.set_static_residency_time_sensor(sens))
    if fall_time := config.get(CONF_FALL_TIME):
        sens = await sensor.new_sensor(fall_time)
        cg.add(parent.set_fall_time_sensor(sens))
    if fall_sensitivity := config.get(CONF_FALL_SENSITIVITY):
        sens = await sensor.new_sensor(fall_sensitivity)
        cg.add(parent.set_fall_sensitivity_sensor(sens))
    if fall_break_height := config.get(CONF_FALL_BREAK_HEIGHT):
        sens = await sensor.new_sensor(fall_break_height)
        cg.add(parent.set_fall_break_height_sensor(sens))
    if track_x := config.get(CONF_TRACK_X):
        sens = await sensor.new_sensor(track_x)
        cg.add(parent.set_track_x_sensor(sens))
    if track_y := config.get(CONF_TRACK_Y):
        sens = await sensor.new_sensor(track_y)
        cg.add(parent.set_track_y_sensor(sens))
    if unmanned_time := config.get(CONF_UNMANNED_TIME):
        sens = await sensor.new_sensor(unmanned_time)
        cg.add(parent.set_unmanned_time_sensor(sens))
    if accumulated_height_duration := config.get(CONF_ACCUMULATED_HEIGHT_DURATION):
        sens = await sensor.new_sensor(accumulated_height_duration)
        cg.add(parent.set_accumulated_height_duration_sensor(sens))
    if seated_distance := config.get(CONF_SEATED_DISTANCE):
        sens = await sensor.new_sensor(seated_distance)
        cg.add(parent.set_seated_distance_sensor(sens))
    if motion_distance := config.get(CONF_MOTION_DISTANCE):
        sens = await sensor.new_sensor(motion_distance)
        cg.add(parent.set_motion_distance_sensor(sens))
    
    # Sleep mode - data
    if in_bed := config.get(CONF_IN_BED):
        sens = await sensor.new_sensor(in_bed)
        cg.add(parent.set_in_bed_sensor(sens))
    if sleep_state := config.get(CONF_SLEEP_STATE):
        sens = await sensor.new_sensor(sleep_state)
        cg.add(parent.set_sleep_state_sensor(sens))
    if wake_duration := config.get(CONF_WAKE_DURATION):
        sens = await sensor.new_sensor(wake_duration)
        cg.add(parent.set_wake_duration_sensor(sens))
    if light_sleep := config.get(CONF_LIGHT_SLEEP):
        sens = await sensor.new_sensor(light_sleep)
        cg.add(parent.set_light_sleep_sensor(sens))
    if deep_sleep := config.get(CONF_DEEP_SLEEP):
        sens = await sensor.new_sensor(deep_sleep)
        cg.add(parent.set_deep_sleep_sensor(sens))
    if sleep_quality := config.get(CONF_SLEEP_QUALITY):
        sens = await sensor.new_sensor(sleep_quality)
        cg.add(parent.set_sleep_quality_sensor(sens))
    if sleep_disturbances := config.get(CONF_SLEEP_DISTURBANCES):
        sens = await sensor.new_sensor(sleep_disturbances)
        cg.add(parent.set_sleep_disturbances_sensor(sens))
    if sleep_quality_rating := config.get(CONF_SLEEP_QUALITY_RATING):
        sens = await sensor.new_sensor(sleep_quality_rating)
        cg.add(parent.set_sleep_quality_rating_sensor(sens))
    if unattended_time := config.get(CONF_UNATTENDED_TIME):
        sens = await sensor.new_sensor(unattended_time)
        cg.add(parent.set_unattended_time_sensor(sens))
    if sleep_deadline := config.get(CONF_SLEEP_DEADLINE):
        sens = await sensor.new_sensor(sleep_deadline)
        cg.add(parent.set_sleep_deadline_sensor(sens))
    if breathe_state := config.get(CONF_BREATHE_STATE):
        sens = await sensor.new_sensor(breathe_state)
        cg.add(parent.set_breathe_state_sensor(sens))
    if breathe_value := config.get(CONF_BREATHE_VALUE):
        sens = await sensor.new_sensor(breathe_value)
